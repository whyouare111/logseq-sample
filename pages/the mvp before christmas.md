...
### 构建一个后端 API 作为 "协议网关"
#### 为了方便用户，让用户获得良好的读写体验，我们决定建立一个后端API，作为客户端和协议之间的 __网关__。这样我们就可以在发布到Arweave之前进行签名验证，或者自己花钱购买Arweave的发布内容，并缓存条目内容，以获得极快的响应时间。这使得我们能够拥有世界级的写作和阅读体验，同时还能实现内容去中心化的所有目标。
